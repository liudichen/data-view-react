...
export interface Decoration10Props extends DecorationCommonProps {}

export const Decoration10 = forwardRef<AutoResizeActions, Decoration10Props>(({ className, style, color }, ref) => {
  const { width, height, domRef } = useAutoResize(ref as any);

  const { animationId1, animationId2, animationId3, animationId4, animationId5, animationId6, animationId7 } = useRef({
  ...
  const classNames = useMemo(() => `dv-decoration-10${className ? ` ${className}` : ""}`, [className]);
